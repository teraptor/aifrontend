...
describe('useAssistentChatStore', () => {
  ...
  describe('createNewSession', () => {
    it('должен создавать новую сессию', async () => {
      ...
      (agentService.createDialog as any).mockResolvedValue(mockResponse);
    ...
    it('должен обрабатывать ошибки при создании сессии', async () => {
      ...
      (agentService.createDialog as any).mockRejectedValue(mockError);
  ...
  describe('addMessage', () => {
    it('должен добавлять сообщение пользователя и получать ответ от ассистента', async () => {
      ...
      (agentService.addMessageToDialog as any).mockResolvedValue(mockResponse);
      ...
      // Проверяем, что сервис был вызван с правильными параметрами
      expect(agentService.addMessageToDialog).toHaveBeenCalledWith(
        'agent1',
        'session1',
