...
import type { GetAgentTemplateListResponse, CreateAgentResponse, MyAgentsResponse, Message } from '@/api/types';
...
describe('agentService', () => {
  ...
  describe('fetchAgentsTemplates', () => {
    it('должен использовать кэш и вернуть шаблоны агентов', async () => {
      const mockResponse: GetAgentTemplateListResponse = {
        data: [
          {
            id: '1',
            name: 'Template 1',
      ...
      (CacheManager.getOrFetch as jest.Mock).mockResolvedValue(mockResponse);
      ...
      expect(CacheManager.getOrFetch).toHaveBeenCalledWith(
        'agentsTemplates',
        expect.any(Function),
    ...
    it('должен обрабатывать ошибки и возвращать пустой массив', async () => {
      ...
      (CacheManager.getOrFetch as jest.Mock).mockRejectedValue(new Error('Тестовая ошибка'));
  ...
  describe('createAgent', () => {
    it('должен создавать нового агента и возвращать его данные', async () => {
      ...
      (CacheManager.getOrFetch as jest.Mock).mockImplementation(
  ...
  describe('getMyAgents', () => {
    it('должен получать список агентов пользователя', async () => {
      const mockResponse: MyAgentsResponse = {
        assistants: [
          {
            id: '1',
            name: 'Agent 1',
